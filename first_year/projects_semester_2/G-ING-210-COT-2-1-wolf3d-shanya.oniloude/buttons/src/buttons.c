...
void button_bases(btn_t *nb)
{
    ...
    SFV2 center = {(nb->button_size.x / 2), (nb->button_size.y / 2)};
...
/*void button_adder(btn_t **buttons, char *tp_nm_txtr_msc[4], SFV2 sz_pos_sc[4])
{
    ...
    new_button->const_button = (tp_nm_txtr_msc[0][0] == 'y') ? 1 : 0;
    new_button->button_name = strdup(tp_nm_txtr_msc[1]);
    new_button->button_texture = SFTCFF(tp_nm_txtr_msc[2], NULL);
    new_button->button_sprite = sfSprite_create();
    new_button->button_size = (sfVector2i){sz_pos_sc[0].x, sz_pos_sc[0].y};
    new_button->button_scales[0] = sz_pos_sc[2];
    new_button->button_scales[1] = sz_pos_sc[3];
    new_button->position = sz_pos_sc[1];
    new_button->clicked_music = sfMusic_createFromFile(tp_nm_txtr_msc[3]);
...
void button_adder(btn_t **buttons, belm_t *elm)
{
    ...
    new_button->button_name = strdup(elm->btn_name);
    new_button->button_texture = SFTCFF(elm->txtr_path, NULL);
    new_button->button_sprite = sfSprite_create();
    new_button->button_size = (sfVector2i){(elm->btn_size).x, (elm->btn_size).y};
