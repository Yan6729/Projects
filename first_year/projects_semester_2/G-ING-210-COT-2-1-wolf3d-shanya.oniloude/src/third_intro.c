...
{
    if (intro->i == 0) {
        sfRectangleShape_setSize(intro->rect_hider, ratior(0, 10));
        sfRectangleShape_setPosition(intro->rect_hider, ratior(70, 1006));
...
{
    int progression = 1000 * global.ratio.x;

    intro->container = sfRectangleShape_getSize(intro->rect_hider);
    intro->angle = sfSprite_getRotation(intro->loading_sprt) - (rand() % 5);
    if (intro->container.x < (1780 * global.ratio.x))
        intro->container.x += (rand() % 23 == 0) ? rand() % progression : 0;
    if (intro->container.x >= (1780 * global.ratio.x)) {
        intro->container.x = 1780 * global.ratio.x;
        intro->i = 2;
