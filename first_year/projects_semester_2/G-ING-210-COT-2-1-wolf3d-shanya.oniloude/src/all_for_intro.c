...
sfRectangleShape *rect_hider_create(void)
{
    ...
    sfRectangleShape_setSize(rect_hider, (SFV2F){(700 * global.ratio.x),
        (300 * global.ratio.y)});
    sfRectangleShape_setFillColor(rect_hider, sfBlack);
    sfRectangleShape_setPosition(rect_hider, (SFV2F){(610 * global.ratio.x),
        (636 * global.ratio.y)});
    return (rect_hider);
}
...
sfSprite *sprt_create(sfTexture *txtr, SFV2F pos)
{
    ...
    sfVector2f new_pos = {(pos.x * global.ratio.x), (pos.y * global.ratio.y)};

    sfSprite_setTexture(sprt, txtr, sfTrue);
    sfSprite_setScale(sprt, global.ratio);
    sfSprite_setPosition(sprt, new_pos);
    return (sprt);
}

void intro_maker_part(intro_t *intro)
{
    sfSprite_setScale(intro->loading_sprt, ratior(0.1, 0.1));
...
intro_t intro_maker(void)
{
    ...
    intro.epi_txtr = sfTexture_createFromFile("img/epitech.png", NULL);
    intro.epi_sprt = sprt_create(intro.epi_txtr, (SFV2F){655, 750});
    intro.epi_msc = sfMusic_createFromFile("snd/epi_intro.mp3");
    intro.shayanx_txtr = sfTexture_createFromFile("img/shayanx.png", NULL);
    intro.shayanx_sprt = sprt_create(intro.shayanx_txtr, (SFV2F){415, 353});
    intro.shayanx_msc = sfMusic_createFromFile("snd/shayanx_intro.mp3");
    intro.shayanx_shoot = sfMusic_createFromFile("snd/intro_shoot.mp3");
    intro.chaos_txtr = sfTexture_createFromFile("img/chaos.png", NULL);
    intro.chaos_sprt = sprt_create(intro.chaos_txtr, (SFV2F){1080, 470});
    intro.loading_txtr = sfTexture_createFromFile("img/loading.png", NULL);
    intro.loading_sprt = sprt_create(intro.loading_txtr, (SFV2F){1832, 966});
    intro.color = (sfColor){.r = 255, .b = 255, .g = 255, .a = 0};
    intro.i = 0;
    intro_maker_part(&intro);
    return (intro);
}
