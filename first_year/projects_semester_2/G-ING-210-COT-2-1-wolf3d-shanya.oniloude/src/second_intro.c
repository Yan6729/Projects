...
void shooting_time(sfRenderWindow *win, intro_t *intro)
{
    ...
    if (intro->i == 3) {
        ...
        sfSprite_setPosition(intro->shayanx_sprt, ratior(850, 353));
    }
    shoot_for_shayanx(intro, 4.5, 3);
    if (intro->i == 4)
        hider_positioner(win, intro, ratior(285, 80), ratior(852, 580));
...
void shayanx_animation(intro_t *intro)
{
    ...
    if (global.seconds >= 6.5) {
        ...
        (intro->container).x -= ((intro->container).x >
            (420 * global.ratio.x)) ? (5 * global.ratio.x) : 0;
